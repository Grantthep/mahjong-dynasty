...
└── audio/         (empty) drop sound files here
```
...
Audio is **optional**. If a file is missing the game stays fully playable and silent.
...
`bgm-*` files. Browsers only start audio after a click, which the game handles for you.
