 ...
 */
export function WinOverlay({ title, subtitle, amount, tone = 'big', onDone }: Props) {
  ...
  useEffect(() => {
    ...
    const duration = reduce ? 1 : COUNT_MS;
    ...
    const tick = (now: number) => {
      ...
      if (t < 1) {
        ...
        holdTimer = window.setTimeout(() => doneRef.current(), HOLD_MS);
      }
    };
    frame = requestAnimationFrame(tick);
    return () => {
